...
//                    }
                    ...
                    for(TransactionInput to: in){
                        try {
                            String pubkey = new String(to.getScriptSig().getPubKeyHash(), StandardCharsets.UTF_8);
                            if(amountaccumulatedperaddress.containsKey(pubkey)){
                                System.out.println("Pubkey found in input transaction------------------------------------------------------");
                            }
                            amountaccumulatedperaddress.remove(pubkey);
                        }catch (Exception e){
                            continue;
                        }
                    }
...
//                    }
        ...
        System.out.println("the total number of addresses: "+amountaccumulatedperaddress.size());
        Coin totalbitcoins = Coin.ZERO;
        for(String s:amountaccumulatedperaddress.keySet()){
            totalbitcoins = totalbitcoins.add(amountaccumulatedperaddress.get(s));

        }

        System.out.println("Total bitcoins value is: "+(totalbitcoins.getValue() * 1.0/100000000));

        System.out.println(amountaccumulatedperaddress);

//        for(String s: amountaccumulatedperaddress.keySet()){
//            Coin value = amountaccumulatedperaddress.get(s);
//            if(value.getValue() * 1.0 /100000000 < 1){
//                amountcategories.put("0-1",amountcategories.get("0-1"));
//            }else if(value.getValue() * 1.0 /100000000 >=1 && value.getValue() * 1.0 /100000000 < 10){
//                amountcategories.put("1-10",amountcategories.get("1-10"));
...
//        }
    ...
    public static void main(String[] args) {
        FindingInputsum wa = new FindingInputsum();
        wa.analyzewallets();
    }
}
